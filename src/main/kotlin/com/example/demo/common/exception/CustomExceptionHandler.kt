...
class CustomExceptionHandler {
    ...
    // 인증되지 않은 요청 (Access Token 누락 또는 만료)
    @ExceptionHandler(AuthorizationDeniedException::class)
    private fun authorizationDeniedException(ex: AuthorizationDeniedException): ResponseEntity<BaseResponse<Map<String, String>>> {
        val errors = mapOf("인증 오류" to "로그인이 필요하거나 접근 권한이 없습니다.")
        return ResponseEntity(
            BaseResponse(ResultCode.ERROR.name, errors, ResultCode.ERROR.msg),
    ...
    private fun missingRequestHeaderException(ex: MissingRequestHeaderException): ResponseEntity<BaseResponse<Map<String, String>>> {
        val errors = mapOf(ex.headerName to "${ex.headerName} 헤더가 필요합니다.")
        return ResponseEntity(
            BaseResponse(ResultCode.ERROR.name, errors, ResultCode.ERROR.msg),
            HttpStatus.BAD_REQUEST
        )
